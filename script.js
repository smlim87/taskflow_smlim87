...
async function syncDataWithSheet() {
    ...
    try {
        // GAS는 redirect를 사용하므로 fetch 옵션을 다음과 같이 설정하는 것이 좋습니다.
        await fetch(GAS_WEB_APP_URL, {
            method: 'POST',
            mode: 'no-cors', // CORS 문제를 피하기 위해 사용 (단, 응답 확인 불가)
            headers: { 'Content-Type': 'text/plain' }, // application/json 대신 사용
            body: JSON.stringify(payload)
        });

        alert("동기화 요청을 보냈습니다! (구글 시트를 확인하세요)");
    } catch (error) {
...
// addTask 내부에서 오류가 나지 않도록 빈 함수라도 정의하거나 호출을 제거하세요.
function saveToGoogleSheet(task) {
    console.log("개별 저장 기능은 아직 구현되지 않았습니다. 전체 동기화를 이용하세요.");
}
...
let editingRecordId = null; // 현재 수정 중인 레코드 ID
let editingRecordColor = null; 
let selectedRecColor = 'bg-yellow-100'; // 새 메모 작성 시 선택된 컬러
let selectedRecordIds = new Set(); // 다중 선택용 Set
let recordClipboard = []; // RECORD 붙여넣기용 내부 클립보드
...
function handleRecordSearch(val) {
    ...
    cards.forEach(card => {
        ...
        if (text.includes(recordSearchQuery)) {
...
// Drag & Drop
function handleRecDragStart(id) { draggedRecordId = id; }
function handleRecDragOver(e) { e.preventDefault(); }
function handleRecDrop(targetId) {
    if (draggedRecordId === targetId) return;
    const draggedIdx = records.findIndex(r => r.id === draggedRecordId);
    const targetIdx = records.findIndex(r => r.id === targetId);
    const [draggedItem] = records.splice(draggedIdx, 1);
    records.splice(targetIdx, 0, draggedItem);
...
function toggleSelectAllRecords() {
    const visibleCards = Array.from(document.querySelectorAll('.record-card')).filter(c => c.style.display !== 'none');
    const visibleIds = visibleCards.map(c => Number(c.dataset.id));
    
    const allSelected = visibleIds.length > 0 && visibleIds.every(id => selectedRecordIds.has(id));
    
    if (allSelected) visibleIds.forEach(id => selectedRecordIds.delete(id));
    else visibleIds.forEach(id => selectedRecordIds.add(id));
...
function deleteSelectedRecords() {
    ...
    if (!confirm(`선택한 ${selectedRecordIds.size}개의 메모를 삭제하시겠습니까?`)) return;
    records = records.filter(r => !selectedRecordIds.has(r.id));
    selectedRecordIds.clear();
    render(); 
}

// [UPDATED] 개별 복사도 내부 클립보드에 저장하도록 수정
function copyRecord(id) {
    const rec = records.find(r => r.id === id);
    
    // 1. 내부 클립보드 저장 (붙여넣기 버튼 활성화용)
    recordClipboard = [{
        title: rec.title,
        content: rec.content,
    ...
    // 2. 외부 클립보드 저장
    const text = `[${rec.title}]\n${rec.content}\n${rec.hashtags.map(t => '#'+t).join(' ')}`;
    
    navigator.clipboard.writeText(text).then(() => {
        alert("메모가 복사되었습니다. (붙여넣기 가능)");
        renderRecordView(document.getElementById('view-container')); // 버튼 갱신을 위해 렌더링
        lucide.createIcons();
    });
}
...
function copySelectedRecords() {
    ...
    recordClipboard = records.filter(r => selectedRecordIds.has(r.id)).map(r => ({
        title: r.title,
        content: r.content,
        hashtags: [...r.hashtags],
        color: r.color
    ...
    const textToCopy = recordClipboard
        .map(r => `[${r.title}]\n${r.content}\n${r.hashtags.map(t => '#'+t).join(' ')}`)
        .join('\n\n---\n\n');
    
    navigator.clipboard.writeText(textToCopy).then(() => {
        alert(`${selectedRecordIds.size}개의 메모가 복사되었습니다.`);
        selectedRecordIds.clear();
        renderRecordView(document.getElementById('view-container'));
        lucide.createIcons();
    });
...
function pasteRecords() {
    if (recordClipboard.length === 0) return;
    
    const newRecords = recordClipboard.map(item => ({
        ...item,
...
function setEditRecordColor(id, color) {
    editingRecordColor = color;
    renderRecordView(document.getElementById('view-container'));
    lucide.createIcons();
}
...
function saveEditRecord(id) {
    ...
    records = records.map(rec => 
        rec.id === id ? { ...rec, title, content, hashtags, color: editingRecordColor } : rec
    );
...
function deleteSelectedTasks() {
    const size = selectedTaskIds.size;
    if (size === 0) return;
    if (!confirm(`정말로 선택한 ${size}개의 업무를 삭제하시겠습니까?`)) return;
    tasks = tasks.filter(task => !selectedTaskIds.has(task.id));
    selectedTaskIds.clear();
    render();
}

function clearSelection() { selectedTaskIds.clear(); render(); }

function copySelectedTasks() {
    clipboard = tasks.filter(t => selectedTaskIds.has(t.id)).map(t => ({ text: t.text, desc: t.desc, category: t.category }));
    selectedTaskIds.clear(); 
    render();
}
...
function pasteTasks() {
    ...
    const newTasks = clipboard.map(item => ({ ...item, id: Date.now() + Math.random(), date: targetDate, completed: false }));
...
function updateFilterBadge() {
    ...
    if (filterTag) { badge.classList.remove('hidden'); name.innerText = `#${filterTag}`; }
...
function toggleTagHide(tag) {
    if (hiddenTags.has(tag)) hiddenTags.delete(tag);
    else { hiddenTags.add(tag); if (filterTag === tag) filterTag = null; }
    render();
}

function toggleHiddenTagsList() { 
    isHiddenTagsListVisible = !isHiddenTagsListVisible; 
    document.getElementById('hashtag-list-hidden').classList.toggle('hidden'); 
    lucide.createIcons(); 
}

function updatePinnedMemo(val) { pinnedMemos[selectedDate.toDateString()] = val; }

// --- View Renderers (Day/Week/Month) ---
function renderDayView(container) {
    ...
    const currentMemo = pinnedMemos[selectedDate.toDateString()] || '';
    
    const dayContent = `
        <div class="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
            <div class="bg-indigo-600 rounded-3xl p-5 mb-6 shadow-lg shadow-indigo-100 relative overflow-hidden">
                <div class="flex items-center gap-2 mb-2"><i data-lucide="pin" class="w-4 h-4 text-indigo-200"></i><span class="text-[11px] font-black text-indigo-100 tracking-widest uppercase">Pinned Memo</span></div>
                <textarea oninput="updatePinnedMemo(this.value)" placeholder="이날의 중요한 메모를 입력하세요..." class="w-full bg-transparent border-none text-white placeholder:text-indigo-300 focus:ring-0 outline-none text-sm font-bold resize-none leading-relaxed">${currentMemo}</textarea>
            </div>
    ...
    document.getElementById('new-task-input').addEventListener('keypress', e => e.key === 'Enter' && addTask());
}
...
function renderWeekView(container) {
    ...
    let weekHtml = `<div class="grid grid-cols-7 gap-1 min-h-[calc(100vh-120px)] bg-slate-200 border-t border-slate-200">`; 
                        ...
                        if (editingId === t.id) {
                            return `
                                <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-3 shadow-sm">
                                    <input id="edit-text-${t.id}" type="text" value="${t.text}" class="w-full bg-white border border-indigo-100 rounded-lg px-2 py-1.5 text-xs font-bold mb-2">
                                    <textarea id="edit-desc-${t.id}" class="w-full bg-white border border-indigo-100 rounded-lg px-2 py-1.5 text-[10px] resize-none mb-2" rows="2">${t.desc || ''}</textarea>
                                    <div class="flex justify-end gap-1">
                                        <button onclick="cancelEdit()" class="p-1 text-[10px] font-bold text-slate-400">취소</button>
                                        <button onclick="saveEdit(${t.id})" class="bg-indigo-600 text-white px-2 py-1 rounded-lg text-[10px] font-bold">저장</button>
                                    </div>
                                </div>`;
                        }
                        return `
...
function updateProgress() {
    ...
    document.getElementById('progress-text').innerText = `${completed}/${todayTasks.length} 완료`;
}
...
function addTask() {
    ...
    tasks.push(newTask);
    saveToGoogleSheet(newTask); // 시트로 전송
...
function toggleTask(id) { tasks = tasks.map(t => t.id === id ? { ...t, completed: !t.completed } : t); render(); }
...
// --- Settings & Modals ---
function openSettings() { document.getElementById('settings-modal').classList.remove('hidden'); updateSettingsUI(); }

function closeSettings() { document.getElementById('settings-modal').classList.add('hidden'); }

function updateSettingsUI() {
    const monBtn = document.getElementById('start-mon');
    const sunBtn = document.getElementById('start-sun');
    monBtn.className = weekStartDay === 1 ? "p-3 rounded-2xl border-2 border-indigo-500 bg-indigo-50 text-indigo-700 font-black" : "p-3 rounded-2xl border-2 border-slate-100 bg-white text-slate-400";
    sunBtn.className = weekStartDay === 0 ? "p-3 rounded-2xl border-2 border-indigo-500 bg-indigo-50 text-indigo-700 font-black" : "p-3 rounded-2xl border-2 border-slate-100 bg-white text-slate-400";
}

function updateWeekStart(val) { weekStartDay = val; render(); }
...
function openCopyModal(id) { 
    ...
    document.getElementById('copy-target-date').valueAsDate = new Date(); 
    document.getElementById('copy-modal').classList.remove('hidden'); 
}

function closeCopyModal() { document.getElementById('copy-modal').classList.add('hidden'); taskToCopy = null; }

function confirmCopy() {
    const dateInput = document.getElementById('copy-target-date').value;
    if (!dateInput) return;
    const date = new Date(dateInput).toDateString();
    tasks.push({ ...taskToCopy, id: Date.now(), date, completed: false });
    closeCopyModal(); 
    render();
}
